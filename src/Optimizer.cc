...
void Optimizer::GlobalBundleAdjustemntWithLines(Map* pMap, int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    vector<KeyFrame*> vpKFs = pMap->GetAllKeyFrames();
    vector<MapPoint*> vpMP = pMap->GetAllMapPoints();
    vector<MapLine*> vpML = pMap->GetAllMapLines();
    BundleAdjustmentWithLines(vpKFs,vpMP,vpML,nIterations,pbStopFlag, nLoopKF, bRobust);
}
...
void Optimizer::BundleAdjustmentWithLines(const vector<KeyFrame *> &vpKFs, const vector<MapPoint *> &vpMP, const vector<MapLine *> &vpML,
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    vector<bool> vbNotIncludedMP;
    vbNotIncludedMP.resize(vpMP.size());
    ...
    const int nExpectedSize_l = (vpKFs.size())*vpML.size();

    vector<ORB_SLAM3::EdgeLineSE3ProjectXYZ*> vpEdgesLine;
    vpEdgesLine.reserve(nExpectedSize_l);

    vector<KeyFrame*> vpEdgeKFLine;
    vpEdgeKFLine.reserve(nExpectedSize_l);

    vector<MapLine*> vpMapLineEdge;
    vpMapLineEdge.reserve(nExpectedSize_l);

    vector<float> Linechi2;
    Linechi2.reserve(nExpectedSize_l);
    ...
    vector<int> initialorbedges(maxKFid+1,0);
    ...
    for(size_t i=0; i<vpML.size(); i++)
    {
        ...
        if(pML->isBad())
            continue;
        g2o::VertexSBALineXYZ* vPoint_l = new g2o::VertexSBALineXYZ();
        vPoint_l->setEstimate(pML->GetWorldPos());
        int id_l = pML->mnId+max+1;
        vPoint_l->setId(id_l);
        vPoint_l->setMarginalized(true);
        optimizer.addVertex(vPoint_l);
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(pKF->isBad() || pKF->mnId>maxKFid)
                continue;
            if(optimizer.vertex(id_l) == NULL || optimizer.vertex(pKF->mnId) == NULL)
                continue;
            nEdges_l++;
            ...
            Eigen::Matrix<double,3,1> obs;
            obs << pKF->mvle_l[mit->second](0), pKF->mvle_l[mit->second](1), pKF->mvle_l[mit->second](2);

            ORB_SLAM3::EdgeLineSE3ProjectXYZ* e = new ORB_SLAM3::EdgeLineSE3ProjectXYZ();
            e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id_l)));
            e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKF->mnId)));
            e->setMeasurement(obs);
            const float &invSigma2 = pKF->mvInvLevelSigma2_l[pKF->mvKeysUn_Line[mit->second].octave];
            e->setInformation(Eigen::Matrix2d::Identity()*Weight*invSigma2);
            ...
            e->obs_temp = pKF->mvle_l[mit->second]; 

            optimizer.addEdge(e);
            vpEdgesLine.push_back(e);
            vpEdgeKFLine.push_back(pKF);
            vpMapLineEdge.push_back(pML);
            Linechi2.push_back(Weight*7.815);
        }

        if(nEdges_l==0)
        {
            optimizer.removeVertex(vPoint_l);
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        else
        {
            ...
            if(dist > 1)
            {
                int numMonoBadPoints = 0, numMonoOptPoints = 0;
                int numStereoBadPoints = 0, numStereoOptPoints = 0;
                int numBadLines = 0, numOptLines = 0;
                vector<MapPoint*> vpMonoMPsOpt, vpStereoMPsOpt;
                vector<MapLine*> vpMLsOpt;
                ...
                {
                    ORB_SLAM3::EdgeLineSE3ProjectXYZ* e = vpEdgesLine[i];
                    MapLine* pML = vpMapLineEdge[i];
                    KeyFrame* pKFedge = vpEdgeKFLine[i];
                    ...
                    if(e->chi2()>Linechi2[i] || !e->isDepthPositive())
                    {
                        numBadLines++;
                    }
                    else
                    {
                        numOptLines++;
                        vpMLsOpt.push_back(pML);
                    }

                }
                Verbose::PrintMess("GBA: KF " + to_string(pKF->mnId) + " had been moved " + to_string(dist) + " meters", Verbose::VERBOSITY_DEBUG);
                Verbose::PrintMess("--Number of observations: " + to_string(numMonoOptPoints) + " in mono, " + to_string(numStereoOptPoints) + " in stereo and " + to_string(numOptLines) + " Lines", Verbose::VERBOSITY_DEBUG);
                Verbose::PrintMess("--Number of discarded observations: " + to_string(numMonoBadPoints) + " in mono, " + to_string(numStereoBadPoints) + " in stereo and " + to_string(numBadLines) + " Lines", Verbose::VERBOSITY_DEBUG);
            }
        }
    }
    Verbose::PrintMess("BA: KFs updated", Verbose::VERBOSITY_DEBUG);
    ...
    for(size_t i=0; i<vpML.size(); i++)
    {
        ...
        g2o::VertexSBALineXYZ* vPoint_l = static_cast<g2o::VertexSBALineXYZ*>(optimizer.vertex(pML->mnId+max+1));

        if(nLoopKF==pMap->GetOriginKF()->mnId)
        {
            pML->SetWorldPos(vPoint_l->estimate().head(3), vPoint_l->estimate().tail(3));
            pML->UpdateNormalAndDepth();
        }
        else
        { 
            pML->mPosGBA = vPoint_l->estimate();
...
int Optimizer::PoseOptimizationPL(Frame *pFrame)
{
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType * linearSolver;
    ...
    // Set MapPoint and MapLine vertices
    const int N = pFrame->N;
    const int N_l = pFrame->N_l;

    vector<ORB_SLAM3::EdgeSE3ProjectXYZOnlyPose*> vpEdgesMono;
    vector<ORB_SLAM3::EdgeLineSE3ProjectXYZOnlyPose*> vpEdgesLine;
    vector<size_t> vnIndexEdgeMono, vnIndexEdgeLine;
    vector<float> invSigma2_Lines;
    vpEdgesMono.reserve(N);
    vpEdgesLine.reserve(N_l);
    vnIndexEdgeMono.reserve(N);
    vnIndexEdgeLine.reserve(N_l);
    invSigma2_Lines.reserve(N_l);
    ...
    for(int i=0; i<N_l; i++)
    {
        ...
        if(pML)
        {
            ...
            Eigen::Matrix<double,3,1> obs;
            obs << pFrame->mvle_l[i](0), pFrame->mvle_l[i](1), pFrame->mvle_l[i](2);

            ORB_SLAM3::EdgeLineSE3ProjectXYZOnlyPose* e = new ORB_SLAM3::EdgeLineSE3ProjectXYZOnlyPose();

            e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(0)));
            e->setMeasurement(obs);
            const float invSigma2 = pFrame->mvInvLevelSigma2_l[pFrame->mvKeysUn_Line[i].octave];
            e->setInformation(Eigen::Matrix2d::Identity()*Weight*invSigma2);
            ...
            Eigen::Vector3d Xw_s = pML->GetWorldPos().head(3);
            Eigen::Vector3d Xw_e = pML->GetWorldPos().tail(3);
            ...
            e->obs_temp = pFrame->mvle_l[i]; 

            optimizer.addEdge(e);

            invSigma2_Lines.push_back(invSigma2);
            vpEdgesLine.push_back(e);
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        for(size_t i=0, iend=vpEdgesLine.size(); i<iend; i++)
        {
            ...
            // Update Information matrix and Huber threshold for the next iteration
            e->setInformation(Eigen::Matrix2d::Identity()*Weight*invSigma2_Lines[i]);
            if(it<2)
            {
            g2o::RobustKernelHuber* rk = new g2o::RobustKernelHuber;
            e->setRobustKernel(rk);
            ...
            if(chi2>chi2Line)
            {                
                pFrame->mvbOutlier_Line[idx]=true;
                e->setLevel(1);
                nBad++;
            }
            else
            {
                pFrame->mvbOutlier_Line[idx]=false;
                e->setLevel(0);
            }
...
int Optimizer::PoseOptimizationOnlyLine(Frame *pFrame)
{
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType * linearSolver;
    ...
    const float deltaLine = sqrt(3.815);
    ...
    const float chi2Line[4]={7.815,7.815,7.815,7.815};
    const int its[4]={10,10,10,10};    
...
int Optimizer::PoseOptimizationOnlyLineAngles(Frame *pFrame)
{
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType * linearSolver;
    ...
    vector<ORB_SLAM3::EdgeLineAngleSE3ProjectXYZOnlyPose*> vpEdgesLine;
    vector<size_t> vnIndexEdgeLine;
    vpEdgesLine.reserve(N_l);
    vnIndexEdgeLine.reserve(N_l);

    const float deltaLine = sqrt(0.00007);
    {
    unique_lock<mutex> lock(MapLine::mGlobalMutex);
    for(int i=0; i<N_l; i++)
    {
        ...
        if(pML)
        {
            ...
            obs << pFrame->mvKeysUn_Line[i].startPointX, pFrame->mvKeysUn_Line[i].startPointY, pFrame->mvKeysUn_Line[i].endPointX, pFrame->mvKeysUn_Line[i].endPointY;

            ORB_SLAM3::EdgeLineAngleSE3ProjectXYZOnlyPose* e = new ORB_SLAM3::EdgeLineAngleSE3ProjectXYZOnlyPose();
            ...
            e->obs_temp[0] = pFrame->mvKeysUn_Line[i].startPointX; 
            e->obs_temp[1] = pFrame->mvKeysUn_Line[i].startPointY; 
            e->obs_temp[2] = pFrame->mvKeysUn_Line[i].endPointX; 
            e->obs_temp[3] = pFrame->mvKeysUn_Line[i].endPointY; 
    ...
    const float chi2Line[4]={0.00007,0.00007,0.00007,0.00007};
    const int its[4]={10,10,10,10};    
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        for(size_t i=0, iend=vpEdgesLine.size(); i<iend; i++)
        {
            ORB_SLAM3::EdgeLineAngleSE3ProjectXYZOnlyPose* e = vpEdgesLine[i];
...
int Optimizer::PoseOptimizationOnlyLineWithAngles(Frame *pFrame)
{
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType * linearSolver;
    ...
    vector<ORB_SLAM3::EdgeLineWithAngleSE3ProjectXYZOnlyPose*> vpEdgesLine;
    vector<size_t> vnIndexEdgeLine;
    vpEdgesLine.reserve(N_l);
    vnIndexEdgeLine.reserve(N_l);

    const float deltaLine = sqrt(1.5);
    ...
    for(int i=0; i<N_l; i++)
    {
        ...
        if(pML)
        {
            ...
            obs << pFrame->mvKeysUn_Line[i].startPointX, pFrame->mvKeysUn_Line[i].startPointY, pFrame->mvKeysUn_Line[i].endPointX, pFrame->mvKeysUn_Line[i].endPointY, pFrame->mvle_l[i](0), pFrame->mvle_l[i](1), pFrame->mvle_l[i](2);

            ORB_SLAM3::EdgeLineWithAngleSE3ProjectXYZOnlyPose* e = new ORB_SLAM3::EdgeLineWithAngleSE3ProjectXYZOnlyPose();

            e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(0)));
            e->setMeasurement(obs);
            const float invSigma2 = pFrame->mvInvLevelSigma2_l[pFrame->mvKeysUn_Line[i].octave];
            Eigen::Matrix4d InformationMatrix = Eigen::Matrix4d::Identity()*invSigma2;
            InformationMatrix(0,0) = InformationMatrix(0,0)/7.815;
            InformationMatrix(1,1) = InformationMatrix(1,1)/7.815;
            InformationMatrix(2,2) = InformationMatrix(2,2)/0.00007;
            InformationMatrix(3,3) = InformationMatrix(3,3)/0.00007;
            ...
            e->obs_temp[2] = pFrame->mvKeysUn_Line[i].endPointX; 
            e->obs_temp[3] = pFrame->mvKeysUn_Line[i].endPointY; 
            e->obs_temp[4] = pFrame->mvle_l[i][0]; 
            e->obs_temp[5] = pFrame->mvle_l[i][1]; 
            e->obs_temp[6] = pFrame->mvle_l[i][2]; 
    ...
    const float chi2Line[4]={1.5,1.5,1.5,1.5};
    const int its[4]={10,10,10,10};    
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        for(size_t i=0, iend=vpEdgesLine.size(); i<iend; i++)
        {
            ORB_SLAM3::EdgeLineWithAngleSE3ProjectXYZOnlyPose* e = vpEdgesLine[i];
...
void Optimizer::LocalBundleAdjustmentPL(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF)
{    
    //cout << "LBA" << endl;
    // Local KeyFrames: First Breath Search from Current Keyframe
    list<KeyFrame*> lLocalKeyFrames;
    ...
    {
        map<KeyFrame*,size_t> observations_l = (*lit)->GetObservations();
        for(map<KeyFrame*,size_t>::iterator mit=observations_l.begin(), mend=observations_l.end(); mit!=mend; mit++)
        {
            KeyFrame* pKFi = mit->first;
    ...
    // Set MapLine vertices
    const int nExpectedSize_l = (lLocalKeyFrames.size()+lFixedCameras.size())*lLocalMapLines.size();
    ...
    vector<float> invSigma2_Line;
    invSigma2_Line.reserve(nExpectedSize_l);
    ...
    for(list<MapLine*>::iterator lit=lLocalMapLines.begin(), lend=lLocalMapLines.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
            {
                // Threshold so as to lower the weight of Lines according to the Number of the initial ORB Edges
                const int thr = 50;
                const int power = initialorbedges[pKFi->mnId]/thr;
                const float Weight = pow(2.0,-power);
                const float thHuberLine = sqrt(Weight*7.815);
                Eigen::Matrix<double,3,1> obs;
                obs << pKFi->mvle_l[mit->second](0), pKFi->mvle_l[mit->second](1), pKFi->mvle_l[mit->second](2);

                ORB_SLAM3::EdgeLineSE3ProjectXYZ* e = new ORB_SLAM3::EdgeLineSE3ProjectXYZ();
                e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id_l)));
                e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKFi->mnId)));
                e->setMeasurement(obs);
                const float &invSigma2_l = pKFi->mvInvLevelSigma2_l[pKFi->mvKeysUn_Line[mit->second].octave];
                e->setInformation(Eigen::Matrix2d::Identity()*Weight*invSigma2_l);
                ...
                e->obs_temp = pKFi->mvle_l[mit->second]; 
                ...
                vpMapLineEdge.push_back(pML);
                Linechi2.push_back(Weight*7.815);
                invSigma2_Line.push_back(invSigma2_l);
                nEdges++;
            }
        }
    }
    ...
    if(bDoMore)
    {
        ...
        for(size_t i=0, iend=vpEdgesMono.size(); i<iend;i++)
        {
            ...
            if(e->chi2()>5.991 || !e->isDepthPositive())
            {
                e->setLevel(1); 
                nMonoBadObs++;
                initialorbedges[vpEdgeKFMono[i]->mnId]--;
            }
        ...
        for(size_t i=0, iend=vpEdgesStereo.size(); i<iend;i++)
        {
            ...
            if(e->chi2()>7.815 || !e->isDepthPositive())
            {
                e->setLevel(1);
                nStereoBadObs++;
                initialorbedges[vpEdgeKFStereo[i]->mnId]--;
            }
        ...
        for(size_t i=0, iend=vpEdgesLine.size(); i<iend;i++)
        {
            ...
            const int thr_n = 50;
            const int power_n = initialorbedges[vpEdgeKFLine[i]->mnId]/thr_n;
            const float Weight_n = pow(2.0,-power_n);
            ...
            // Update the information Matrix for the next iteration
            e->setInformation(Eigen::Matrix2d::Identity()*Weight_n*invSigma2_Line[i]);
            Linechi2[i]=Weight_n*7.815;
    ...
    vector<pair<KeyFrame*,MapLine*> > vToErase_l;
    vToErase_l.reserve(vpEdgesLine.size());
    ...
    for(size_t i=0, iend=vpEdgesLine.size(); i<iend;i++)
    {
        ...
        if(e->chi2()>Linechi2[i] || !e->isDepthPositive())
        {
            ...
            vToErase_l.push_back(make_pair(pKFi,pML));
        }
    }

    //Verbose::PrintMess("LM-LBA: outlier observations: " + to_string(vToErase.size()), Verbose::VERBOSITY_DEBUG);
    bool bRedrawError = false;
    if(vToErase.size()+vToErase_l.size() >= (vpMapPointEdgeMono.size()+vpMapPointEdgeStereo.size()+vpMapLineEdge.size()) * 0.5)
    {
        Verbose::PrintMess("LM-LBA: ERROR IN THE OPTIMIZATION, MOST OF THE POINTS HAS BECOME OUTLIERS", Verbose::VERBOSITY_NORMAL);
    ...
    if(!vToErase_l.empty())
    {
        for(size_t i=0;i<vToErase_l.size();i++)
        {
            KeyFrame* pKFi = vToErase_l[i].first;
            MapLine* pMLi = vToErase_l[i].second;
...
void Optimizer::LocalBundleAdjustmentOnlyLines(KeyFrame *pKF, bool* pbStopFlag, Map* pMap)
{    
    // Local KeyFrames: First Breath Search from Current Keyframe
    list<KeyFrame*> lLocalKeyFrames;
    ...
    const float thHuberLine = sqrt(7.815);
    ...
    for(list<MapLine*>::iterator lit=lLocalMapLines.begin(), lend=lLocalMapLines.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            {                
                ...
                e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKFi->mnId)));
                e->setMeasurement(obs);
                const float &invSigma2 = pKFi->mvInvLevelSigma2_l[pKFi->mvKeysUn_Line[mit->second].octave];
                e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
    ...
    vToErase.reserve(vpEdgesLine.size());
    ...
    {
        MapLine* pML = *lit;
        g2o::VertexSBALineXYZ* vPoint_l = static_cast<g2o::VertexSBALineXYZ*>(optimizer.vertex(pML->mnId+maxKFid+1));
        pML->SetWorldPos(vPoint_l->estimate().head(3), vPoint_l->estimate().tail(3));
        pML->UpdateNormalAndDepth();
    }
}

void Optimizer::LocalBundleAdjustmentOnlyLinesAngle(KeyFrame *pKF, bool* pbStopFlag, Map* pMap)
{    
    // Local KeyFrames: First Breath Search from Current Keyframe
    list<KeyFrame*> lLocalKeyFrames;
    ...
    vector<ORB_SLAM3::EdgeLineAngleSE3ProjectXYZ*> vpEdgesLine;
    vpEdgesLine.reserve(nExpectedSize_l);
    ...
    const float thHuberLine = sqrt(0.00007);
    ...
    for(list<MapLine*>::iterator lit=lLocalMapLines.begin(), lend=lLocalMapLines.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            {                
                Eigen::Matrix<double,4,1> obs;
                obs << pKFi->mvKeysUn_Line[mit->second].startPointX, pKFi->mvKeysUn_Line[mit->second].startPointY, pKFi->mvKeysUn_Line[mit->second].endPointX, pKFi->mvKeysUn_Line[mit->second].endPointY;

                ORB_SLAM3::EdgeLineAngleSE3ProjectXYZ* e = new ORB_SLAM3::EdgeLineAngleSE3ProjectXYZ();
                e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id_l)));
                e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKFi->mnId)));
                e->setMeasurement(obs);
                const float &invSigma2 = pKFi->mvInvLevelSigma2_l[pKFi->mvKeysUn_Line[mit->second].octave];
                ...
                e->obs_temp[0] = pKFi->mvKeysUn_Line[mit->second].startPointX; 
                e->obs_temp[1] = pKFi->mvKeysUn_Line[mit->second].startPointY; 
                e->obs_temp[2] = pKFi->mvKeysUn_Line[mit->second].endPointX; 
                e->obs_temp[3] = pKFi->mvKeysUn_Line[mit->second].endPointY; 
    ...
    for(size_t i=0, iend=vpEdgesLine.size(); i<iend;i++)
    {
        ORB_SLAM3::EdgeLineAngleSE3ProjectXYZ* e = vpEdgesLine[i];
        MapLine* pML = vpMapLineEdge[i];

        if(pML->isBad())
            continue;

        if(e->chi2()>0.00007 || !e->isDepthPositive())
        {
            e->setLevel(1);
        }
...
void Optimizer::LocalBundleAdjustmentOnlyLinesWithAngle(KeyFrame *pKF, bool* pbStopFlag, Map* pMap)
{    
    // Local KeyFrames: First Breath Search from Current Keyframe
    list<KeyFrame*> lLocalKeyFrames;
    ...
    vector<ORB_SLAM3::EdgeLineWithAngleSE3ProjectXYZ*> vpEdgesLine;
    vpEdgesLine.reserve(nExpectedSize_l);
    ...
    const float thHuberLine = sqrt(1.5);
    ...
    for(list<MapLine*>::iterator lit=lLocalMapLines.begin(), lend=lLocalMapLines.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            {                
                Eigen::Matrix<double,7,1> obs;
                obs << pKFi->mvKeysUn_Line[mit->second].startPointX, pKFi->mvKeysUn_Line[mit->second].startPointY, pKFi->mvKeysUn_Line[mit->second].endPointX, pKFi->mvKeysUn_Line[mit->second].endPointY, pKFi->mvle_l[mit->second](0), pKFi->mvle_l[mit->second](1), pKFi->mvle_l[mit->second](2);

                ORB_SLAM3::EdgeLineWithAngleSE3ProjectXYZ* e = new ORB_SLAM3::EdgeLineWithAngleSE3ProjectXYZ();
                e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id_l)));
                e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKFi->mnId)));
                e->setMeasurement(obs);
                const float &invSigma2 = pKFi->mvInvLevelSigma2_l[pKFi->mvKeysUn_Line[mit->second].octave];
                ...
                e->obs_temp[2] = pKFi->mvKeysUn_Line[mit->second].endPointX; 
                e->obs_temp[3] = pKFi->mvKeysUn_Line[mit->second].endPointY; 
                e->obs_temp[4] = pKFi->mvle_l[mit->second](0);
                e->obs_temp[5] = pKFi->mvle_l[mit->second](1);
                e->obs_temp[6] = pKFi->mvle_l[mit->second](2);
    ...
    for(size_t i=0, iend=vpEdgesLine.size(); i<iend;i++)
    {
        ORB_SLAM3::EdgeLineWithAngleSE3ProjectXYZ* e = vpEdgesLine[i];
        MapLine* pML = vpMapLineEdge[i];

        if(pML->isBad())
            continue;

        if(e->chi2()>1.5 || !e->isDepthPositive())
        {
            e->setLevel(1);
        }
...
void Optimizer::LocalBundleAdjustmentPL(KeyFrame* pMainKF,vector<KeyFrame*> vpAdjustKF, vector<KeyFrame*> vpFixedKF, bool *pbStopFlag)
{
    bool bShowImages = false;
    ...
    for(KeyFrame* pKFi : vpFixedKF)
    {
        ...
        set<MapLine*> spViewMLs = pKFi->GetMapLines();
        for(MapLine* pMLi : spViewMLs)
        {
            if(pMLi)
    ...
    const int nExpectedSize_l = (vpAdjustKF.size()+vpFixedKF.size())*vpMLs.size();
    ...
    for(vector<MapLine*>::iterator lit=vpMLs.begin(), lend=vpMLs.end(); lit!=lend; lit++)
    {
        MapLine* pML = *lit;
        g2o::VertexSBALineXYZ* vPoint_l = new g2o::VertexSBALineXYZ();
        vPoint_l->setEstimate(pML->GetWorldPos());
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(pKFi->isBad() || pKFi->mnId>maxKFid || pKFi->mnBALocalForMerge != pMainKF->mnId || !pKFi->GetMapLine(mit->second))
                continue;
    ...
    if(bDoMore)
    {
        ...
        // Check inlier observations
        int badMonoMP = 0, badStereoMP = 0, badLine = 0;
        for(size_t i=0, iend=vpEdgesMono.size(); i<iend;i++)
        {
            ORB_SLAM3::EdgeSE3ProjectXYZ* e = vpEdgesMono[i];
            MapPoint* pMP = vpMapPointEdgeMono[i];
    ...
    vToErase_l.reserve(vpEdgesLine.size());
    set<MapPoint*> spErasedMPs;
    set<MapLine*> spErasedMLs;
    set<KeyFrame*> spErasedKFs;
    set<KeyFrame*> spErasedKFs_l;
