...
void LoopClosing::CorrectLoopWithLines()
{
...

    {
        ...
        for(KeyFrameAndPose::iterator mit=CorrectedSim3.begin(), mend=CorrectedSim3.end(); mit!=mend; mit++)
        {
            ...
            // Correct all MapLines obsrved by current keyframe and neighbors, so that they align with the other side of the loop
            ...
            for(size_t iML=0, endMLi = vpMLsi.size(); iML<endMLi; iML++)
            {
                ...
                // Project with non-corrected pose and project back with corrected pose
                Eigen::Matrix<double,3,1> eigCorrectedP3Dw_sp = g2oCorrectedSwi.map(g2oSiw.map(pMLi->GetWorldPos().head(3)));
                Eigen::Matrix<double,3,1> eigCorrectedP3Dw_ep = g2oCorrectedSwi.map(g2oSiw.map(pMLi->GetWorldPos().tail(3)));
    ...
    if(!pLoopMap->isImuInitialized() || (pLoopMap->KeyFramesInMap()<200 && mpAtlas->CountMaps()==1))
    {
        ...
        // TO DO FIX RunGlobalBundleAdjustmentWithLines
        //mpThreadGBA = new thread(&LoopClosing::RunGlobalBundleAdjustmentWithLines, this, pLoopMap, mpCurrentKF->mnId);
    }
...
void LoopClosing::MergeLocalWithLines()
{
    ...
    for(KeyFrame* pKFi : spLocalWindowKFs)
    {
        ...
        set<MapLine*> spMLs = pKFi->GetMapLines();
        spLocalWindowMPs.insert(spMPs.begin(), spMPs.end());
        spLocalWindowMLs.insert(spMLs.begin(), spMLs.end());
    }
    ...
    Verbose::PrintMess("MERGE-VISUAL: Number of MLs in local window from active map: " + to_string(spLocalWindowMLs.size()), Verbose::VERBOSITY_DEBUG);
    ...
    Verbose::PrintMess("MERGE-VISUAL: Number of MLs in the active map: " + to_string(pCurrentMap->GetAllMapLines().size()), Verbose::VERBOSITY_DEBUG);
    ...
    for(MapLine* pMLi : spLocalWindowMLs)
    {
        ...
        // Project with non-corrected pose and project back with corrected pose
        Eigen::Matrix<double,3,1> eigP3Dw_sp = pMLi->GetWorldPos().head(3);
        Eigen::Matrix<double,3,1> eigP3Dw_ep = pMLi->GetWorldPos().tail(3);
        Eigen::Matrix<double,3,1> eigCorrectedP3Dw_sp = g2oCorrectedSwi.map(g2oNonCorrectedSiw.map(eigP3Dw_sp));
        Eigen::Matrix<double,3,1> eigCorrectedP3Dw_ep = g2oCorrectedSwi.map(g2oNonCorrectedSiw.map(eigP3Dw_ep));
        Eigen::Matrix3d eigR = g2oCorrectedSwi.rotation().toRotationMatrix();
        Eigen::Matrix3d Rcor = eigR * g2oNonCorrectedSiw.rotation().toRotationMatrix();

        pMLi->mPosMerge.head(3) = eigCorrectedP3Dw_sp;
        pMLi->mPosMerge.tail(3) = eigCorrectedP3Dw_ep;

        pMLi->mNormalVectorMerge = Converter::toCvMat(Rcor) * pMLi->GetNormal();
        //pMLi->SetWorldPos(cvCorrectedP3Dw);
        //pMLi->UpdateMap(pMergeMap);
        //pMergeMap->AddMapLine(pMLi);
        //pCurrentMap->EraseMapLinw(pMLi);
        //pMLi->UpdateNormalAndDepth();
    }
...
#endif
    {
        ...
        for(MapLine* pMLi : spLocalWindowMLs)
        {
            ...
            pMLi->SetWorldPos(pMLi->mPosMerge.head(3),pMLi->mPosMerge.tail(3));
    ...
    else
    {
        ...
        // TO DO FINISH AND TEST Optimizer::LocalBundleAdjustmentPL(mpCurrentKF, vpLocalCurrentWindowKFs, vpMergeConnectedKFs,&bStop);
        // Optimizer::LocalBundleAdjustmentPL(mpCurrentKF, vpLocalCurrentWindowKFs, vpMergeConnectedKFs,&bStop);
    }
    ...
    vector<MapLine*> vpCurrentMapMLs = pCurrentMap->GetAllMapLines();
    ...
    else
    {
...

        {
            ...
            Verbose::PrintMess("MERGE-VISUAL: There are " + to_string(pMergeMap->MapPointsInMap()) + " MPs in the map", Verbose::VERBOSITY_DEBUG);
            Verbose::PrintMess("MERGE-VISUAL: There are " + to_string(pMergeMap->MapLinesInMap()) + " MLs in the map", Verbose::VERBOSITY_DEBUG);
        }
    ...
    if(bRelaunchBA && (!pCurrentMap->isImuInitialized() || (pCurrentMap->KeyFramesInMap()<200 && mpAtlas->CountMaps()==1)))
    {
        ...
        // TO DO FIX RunGlobalBundleAdjustmentWithLines
        //mpThreadGBA = new thread(&LoopClosing::RunGlobalBundleAdjustmentWithLines,this, pMergeMap, mpCurrentKF->mnId);
    }
...
void LoopClosing::RunGlobalBundleAdjustmentWithLines(Map* pActiveMap, unsigned long nLoopKF)
{
    Verbose::PrintMess("Starting Global Bundle Adjustment", Verbose::VERBOSITY_NORMAL);

    const bool bImuInit = pActiveMap->isImuInitialized();

    Optimizer::GlobalBundleAdjustemntWithLines(pActiveMap,10,&mbStopGBA,nLoopKF,false);
        ...
        if(!mbStopGBA)
        {
            ...
            for(size_t i=0; i<vpMLs.size(); i++)
            {
                ...
                else
                {
                    ...
                    cv::Mat Xc_sp = Rcw*Converter::toCvMat(sp)+tcw;
                    cv::Mat Xc_ep = Rcw*Converter::toCvMat(ep)+tcw;
                    ...
                    Eigen::Vector3d corrected_sp = Converter::toVector3d(Rwc*Xc_sp+twc);
                    Eigen::Vector3d corrected_ep = Converter::toVector3d(Rwc*Xc_ep+twc);

                    pML->SetWorldPos(corrected_sp,corrected_ep);
                }
            }
