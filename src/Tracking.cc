    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr)
{
    ...
    {
        b_parse_line = ParseLineParamFile(fSettings);
        if(!b_parse_line)
        {
...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    if (mSensor == System::STEREO && !mpCamera2)
        mCurrentFrame = Frame(mImGray,imGrayRight,timestamp,mpORBextractorLeft,mpORBextractorRight, mpLineextractorLeft, mpLineextractorRight,mpORBVocabulary, mpLineVocabulary, mK,mDistCoef,mbf,mThDepth,mpCamera);
    ...
    else
    {
        ...
        std::chrono::steady_clock::time_point timeStartPosePredict = std::chrono::steady_clock::now();
            ...
            if(mState==OK)
            {
                ...
                CheckReplacedInLastFrameWithLines();
                ...
                {
                    //Verbose::PrintMess("TRACK: Track with motion model", Verbose::VERBOSITY_DEBUG);
                    bOK = TrackWithMotionModelWithLines();
                    if(!bOK)
                        bOK = TrackReferenceKeyFrameWithLines();
                }
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(!mVelocity.empty())
                    {
                        bOKMM = TrackWithMotionModelWithLines();
                        vpMPsMM = mCurrentFrame.mvpMapPoints;
                        vbOutMM = mCurrentFrame.mvbOutlier;
                        TcwMM = mCurrentFrame.mTcw.clone();
                    }
        ...
        if(bOK || mState==RECENTLY_LOST)
        {
            ...
            std::chrono::steady_clock::time_point timeStartNewKF = std::chrono::steady_clock::now();
#endif
            bool bNeedKF = NeedNewKeyFrameWithLines();
#ifdef SAVE_TIMES
            std::chrono::steady_clock::time_point timeEndNewKF = std::chrono::steady_clock::now();
...
bool Tracking::TrackReferenceKeyFrameWithLines()
{
    ...
    for (int i1 = 0; i1 < nmatches_12; ++i1) {
        ...
        /*if(true) {
            ...
            const float sX_last = mpReferenceKF->mvKeysUn_Line[i1].startPointX;
            const float sY_curr = mCurrentFrame.mvKeysUn_Line[i2].startPointY;
            const float sY_last = mpReferenceKF->mvKeysUn_Line[i1].startPointY;
            const float eX_curr = mCurrentFrame.mvKeysUn_Line[i2].endPointX;
            const float eX_last = mpReferenceKF->mvKeysUn_Line[i1].endPointX;
            const float eY_curr = mCurrentFrame.mvKeysUn_Line[i2].endPointY;
            const float eY_last = mpReferenceKF->mvKeysUn_Line[i1].endPointY;
...
bool Tracking::NeedNewKeyFrameWithLines()
{
    if(((mSensor == System::IMU_MONOCULAR) || (mSensor == System::IMU_STEREO)) && !mpAtlas->GetCurrentMap()->isImuInitialized())
    {
        if (mSensor == System::IMU_MONOCULAR && (mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp)>=0.5)
            return true;
        else if (mSensor == System::IMU_STEREO && (mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp)>=0.5)
            return true;
        else
            return false;
    }
    ...
    const bool c1b = ((mCurrentFrame.mnId>=mnLastKeyFrameId+mMinFrames) && bLocalMappingIdle);
    //Condition 1c: tracking is weak
    const bool c1c = mSensor!=System::MONOCULAR && mSensor!=System::IMU_MONOCULAR && mSensor!=System::IMU_STEREO && (mnMatchesInliers<nRefMatches*0.15 || bNeedToInsertClose) ;
    // Condition 2: Few tracked points compared to reference keyframe. Lots of visual odometry compared to map matches.
    const bool c2 = (((mnMatchesInliers<nRefMatches*thRefRatio || bNeedToInsertClose)) && mnMatchesInliers>10);
...
void Tracking::CreateNewKeyFrameWithLines()
{
    if(mpLocalMapper->IsInitializing())
        return;
